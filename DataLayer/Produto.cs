...
namespace DataLayer
{
    public class Produto
    {
            ...
            ref DateTime? dataVenda, ref int? clienteId, out string erro)
        {
            ...
            try
            {
                ...
                if (dr.Read())
                {
                    ...
                    dataVenda = dr["DataVenda"] == DBNull.Value
                        ? null
                        : Convert.ToDateTime(dr["DataVenda"]);

                    clienteId = dr["ClienteId"] == DBNull.Value
                        ? null
                        : Convert.ToInt32(dr["ClienteId"]);
        ...
        {
            return BaseDadosGlobal.ObterLista("Listar_Produto", out erro);
        }

        public static DataTable ListarCompleto(out string erro)
        {
            return BaseDadosGlobal.ObterLista("Listar_ProdutoCompleto", out erro);
        }
    }
}